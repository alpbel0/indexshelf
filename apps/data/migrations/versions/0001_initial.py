...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("execution_version", sa.BigInteger(), nullable=False, server_default="1"),
        ...
        sa.UniqueConstraint("job_id", name="uq_job_execution_records_job_id"),
    )
    op.create_index(
        "ix_job_execution_records_state_available_at",
        "job_execution_records",
        ["state", "available_at"],
    )
    op.create_table(
        "extraction_attempt_records",
        sa.Column("id", uuid, primary_key=True),
        sa.Column(
            "execution_id",
            uuid,
            sa.ForeignKey("job_execution_records.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_attempt_records_state_created_at",
        "extraction_attempt_records",
        ["state", "created_at"],
    )
    op.create_table(
        ...
        sa.Column("created_at", ts, nullable=False, server_default=sa.func.now()),
        sa.UniqueConstraint("event_id", name="uq_event_outbox_records_event_id"),
    )
    op.create_index(
        "ix_event_outbox_records_pending",
        "event_outbox_records",
...
def downgrade() -> None:
    ...
    op.drop_table("extraction_attempt_records")
    op.drop_table("job_execution_records")
